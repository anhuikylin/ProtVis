...
golem_add_external_resources <- function() {
  ...
  shiny::tags$head(
    shiny::tags$script(shiny::HTML("
      (function () {
        function unlockProtVisButton(button) {
          if (!button) return;
        ...
        function unlockProtVisButtons() {
          document.querySelectorAll('button.pv-run-button[data-pv-running=\"true\"]').forEach(function (button) {
            unlockProtVisButton(button);
          });
        }

        function registerProtVisRunButtonHandler() {
          if (!window.Shiny || window.protvisRunButtonHandlerRegistered) return;
          window.Shiny.addCustomMessageHandler('protvis-unlock-run-button', function (message) {
            if (message && message.id) {
              unlockProtVisButton(document.getElementById(message.id));
            } else {
              unlockProtVisButtons();
            }
          });
          window.protvisRunButtonHandlerRegistered = true;
        }
        ...
        registerProtVisRunButtonHandler();
        document.addEventListener('DOMContentLoaded', registerProtVisRunButtonHandler);
        document.addEventListener('shiny:connected', registerProtVisRunButtonHandler);
        document.addEventListener('shiny:idle', unlockProtVisButtons);
        if (window.jQuery) {
          window.jQuery(document).on('shiny:connected.protvisRunButtons', registerProtVisRunButtonHandler);
          window.jQuery(document).on('shiny:idle.protvisRunButtons', unlockProtVisButtons);
        }
      }());
    ")),
    shiny::tags$link(
  ...
  shiny::div(
    ...
    shiny::tags$section(
      ...
      shiny::div(
        ...
        shiny::div(
          class = "pv-pill-row",
          shiny::span(class = "pv-pill", "Unified project data"),
      ...
      shiny::div(
        ...
        shiny::div(
          ...
          shiny::tags$strong("Project dataset"),
          shiny::tags$small(
            "Inputs, results, and history stay connected"
          )
        ),
    ...
    shiny::div(
      ...
      shiny::div(class = "pv-metric", shiny::tags$strong("1"), shiny::span("connected project dataset"))
    ),
    shiny::tags$section(
      class = "pv-section",
      shiny::div(
      ...
      shiny::div(
        ...
        shiny::div(
          ...
          shiny::h3("Tabular proteomics results"),
          shiny::p("Import MaxQuant, Proteome Discoverer, DIA-NN, Spectronaut, FragPipe, Skyline, OpenMS, or a custom matrix. MaxQuant outputs can first be organized with MaxQuant Output Preparation."),
          shiny::tags$small("Project init → Prepare data when needed → Pre-processing → Downstream analysis")
        )
      )
    ),
    shiny::tags$section(
    ...
    shiny::tags$section(
      ...
      shiny::div(
        ...
        shiny::p("Every completed stage contributes data, parameters, status, and history to the same project dataset, enabling inspection and checkpoint recovery.")
      ),
      shiny::div(
        class = "pv-workflow",
        workflow_step(1, "Project init", "Set the working directory and confirm metadata."),
        workflow_step(2, "Input route", "Search raw data with Sage or import a supported results table."),
    ...
    shiny::tags$section(
      ...
      shiny::div(
        ...
        shiny::span(class = "pv-section-index", "05 · Project data"),
        shiny::h2("Everything stays connected"),
        shiny::p("ProtVis keeps quantitative data aligned with sample and protein information while recording each processing decision. You can inspect completed work and continue from recoverable checkpoints without managing separate files between steps."),
        shiny::div(
          class = "pv-proof-list",
